/*
  ...
  Archivo: bl.carreras.js
  Funcion: conector local para el modulo Carreras.
*/
(function (window) {
  'use strict';
  ...
  const MODULE = Object.freeze({
    id: 'carreras',
    nombre: 'Carreras',
    ruta: 'Curriculo/carreras',
    camposClave: ['codigoCarrera', 'nombreCarrera', 'modalidad', 'sede', 'estadoCarrera']
  });
  ...
  function normalize(item) {
    ...
    return BL.schema?.normalizeRecord(data, {
      modulo: MODULE.id,
      ruta: MODULE.ruta,
      nombre: data.nombre || data.nombreCarrera || data.carrera || data.codigoCarrera || 'carrera-sin-nombre',
      origen: data.origen || 'modulo-carreras'
    });
  }
  ...
  async function save(item) {
    ...
    if (!validation.valido) {
      await BL.storage?.appendToArray?.('errores', 'errores', {
        id: BL.utils?.makeId ? BL.utils.makeId('error-carreras') : `error-carreras-${Date.now()}`,
        tipo: 'validacion-carreras',
        mensaje: 'Registro de carrera invalido.',
        detalle: validation.errores,
        registro: record,
        creadoEn: BL.utils?.nowISO ? BL.utils.nowISO() : new Date().toISOString()
      });
      BL.logger?.error('Carreras: registro invalido.', validation.errores);
    ...
    await BL.storage?.appendToArray?.('historial', 'historial', {
      id: BL.utils?.makeId ? BL.utils.makeId('hist-carreras') : `hist-carreras-${Date.now()}`,
    ...
    BL.logger?.info('Carreras: registro guardado en Base local.', { clave: key });
  ...
  BL.modulos.carreras = { config: MODULE, normalize, save, saveMany, list };
})(window);
