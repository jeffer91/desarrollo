/*
  ...
  Archivo: bl.control.js
  Funcion: conector local para el modulo Control.
*/
(function (window) {
  'use strict';
  ...
  const MODULE = Object.freeze({
    id: 'control',
    nombre: 'Control',
    ruta: 'Curriculo/control',
    camposClave: ['tipoControl', 'estadoControl', 'responsable']
  });
  ...
  function normalize(item) {
    ...
    return BL.schema?.normalizeRecord(data, {
      modulo: MODULE.id,
      ruta: MODULE.ruta,
      nombre: data.nombre || data.tipoControl || data.estadoControl || 'control-sin-nombre',
      origen: data.origen || 'modulo-control'
    });
  }
  ...
  async function save(item) {
    ...
    if (!validation.valido) {
      await BL.storage?.appendToArray?.('errores', 'errores', {
        id: BL.utils?.makeId ? BL.utils.makeId('error-control') : `error-control-${Date.now()}`,
        tipo: 'validacion-control',
        mensaje: 'Registro de control invalido.',
        detalle: validation.errores,
        registro: record,
        creadoEn: BL.utils?.nowISO ? BL.utils.nowISO() : new Date().toISOString()
      });
      BL.logger?.error('Control: registro invalido.', validation.errores);
    ...
    await BL.storage?.appendToArray?.('historial', 'historial', {
      id: BL.utils?.makeId ? BL.utils.makeId('hist-control') : `hist-control-${Date.now()}`,
    ...
    BL.logger?.info('Control: registro guardado en Base local.', { clave: key });
  ...
  BL.modulos.control = { config: MODULE, normalize, save, saveMany, list };
})(window);
