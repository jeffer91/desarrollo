/*
  ...
  Archivo: bl.materias.js
  Funcion: conector local para el modulo Materias.
*/
(function (window) {
  'use strict';
  ...
  const MODULE = Object.freeze({
    id: 'materias',
    nombre: 'Materias',
    ruta: 'Curriculo/materias',
    camposClave: ['codigoMateria', 'nombreMateria', 'nivel', 'creditos', 'horas']
  });
  ...
  function normalize(item) {
    ...
    return BL.schema?.normalizeRecord(data, {
      modulo: MODULE.id,
      ruta: MODULE.ruta,
      nombre: data.nombre || data.nombreMateria || data.materia || data.codigoMateria || 'materia-sin-nombre',
      origen: data.origen || 'modulo-materias'
    });
  }
  ...
  async function save(item) {
    ...
    if (!validation.valido) {
      await BL.storage?.appendToArray?.('errores', 'errores', {
        id: BL.utils?.makeId ? BL.utils.makeId('error-materias') : `error-materias-${Date.now()}`,
        tipo: 'validacion-materias',
        mensaje: 'Registro de materia invalido.',
        detalle: validation.errores,
        registro: record,
        creadoEn: BL.utils?.nowISO ? BL.utils.nowISO() : new Date().toISOString()
      });
      BL.logger?.error('Materias: registro invalido.', validation.errores);
    ...
    await BL.storage?.appendToArray?.('historial', 'historial', {
      id: BL.utils?.makeId ? BL.utils.makeId('hist-materias') : `hist-materias-${Date.now()}`,
    ...
    BL.logger?.info('Materias: registro guardado en Base local.', { clave: key });
  ...
  BL.modulos.materias = { config: MODULE, normalize, save, saveMany, list };
})(window);
