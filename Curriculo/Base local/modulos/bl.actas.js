/*
  ...
  Archivo: bl.actas.js
  Funcion: conector local para el modulo Actas.
*/
(function (window) {
  'use strict';
  ...
  const MODULE = Object.freeze({
    id: 'actas',
    nombre: 'Actas',
    ruta: 'Curriculo/actas',
    camposClave: ['codigoActa', 'fechaActa', 'tipoActa', 'responsable']
  });
  ...
  function normalize(item) {
    ...
    return BL.schema?.normalizeRecord(data, {
      modulo: MODULE.id,
      ruta: MODULE.ruta,
      nombre: data.nombre || data.codigoActa || data.tipoActa || data.acta || 'acta-sin-nombre',
      origen: data.origen || 'modulo-actas'
    });
  }
  ...
  async function save(item) {
    ...
    if (!validation.valido) {
      await BL.storage?.appendToArray?.('errores', 'errores', {
        id: BL.utils?.makeId ? BL.utils.makeId('error-actas') : `error-actas-${Date.now()}`,
        tipo: 'validacion-actas',
        mensaje: 'Registro de acta invalido.',
        detalle: validation.errores,
        registro: record,
        creadoEn: BL.utils?.nowISO ? BL.utils.nowISO() : new Date().toISOString()
      });
      BL.logger?.error('Actas: registro invalido.', validation.errores);
    ...
    await BL.storage?.appendToArray?.('historial', 'historial', {
      id: BL.utils?.makeId ? BL.utils.makeId('hist-actas') : `hist-actas-${Date.now()}`,
    ...
    BL.logger?.info('Actas: registro guardado en Base local.', { clave: key });
  ...
  BL.modulos.actas = { config: MODULE, normalize, save, saveMany, list };
})(window);
