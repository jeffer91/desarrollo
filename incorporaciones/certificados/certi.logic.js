...
Función o funciones:
- Procesar registros del Excel de mejores egresados.
- Elegir automáticamente el mayor promedio por carrera.
- No depender del catálogo externo para aceptar o rechazar carreras.
- Usar el Excel como fuente principal para carreras, nombres y promedios.
...
(function () {
  ...
  function procesar(registros, opciones) {
    ...
    Object.keys(grupos).forEach(function (carreraOficial) {
      ...
      if (candidatosMaximos.length > 1) {
        ...
        const elegido = candidatosMaximos.find(function (registro) {
          return Number(registro.indice) === Number(indiceSeleccionado);
        });
  ...
  function obtenerCarreraDesdeRegistro(registro) {
    ...
    const desdeRaw = extraerCarreraProbable(textos);
  ...
  function construirAlertas(datos) {
    ...
    if (!datos.registros || datos.registros.length === 0) {
      alertas.push({
        ...
        mensaje: "El Excel no contiene registros válidos para procesar."
      });
    }
  ...
  function puntuarCarrera(valor) {
    ...
    if (clave.includes("SEGURIDAD")) puntos += 1000;
    if (clave.includes("RIESGO")) puntos += 900;
    if (clave.includes("PREVENCION")) puntos += 800;
    if (clave.includes("TECNOLOGIA")) puntos += 100;
    if (clave.includes("SUPERIOR")) puntos += 80;

    return puntos + clave.length;
  }

  function contieneSeguridadRiesgos(valor) {
    const clave = claveTexto(valor);
    return (
  ...
  function contieneSeguridadCiudadana(valor) {
    const clave = claveTexto(valor);
    return clave.includes("SEGURIDAD") && clave.includes("CIUDADANA");
  }
  ...
  function quitarPrefijoCarrera(nombre) {
    return limpiarTexto(nombre)
      ...
      .replace(/^T[EÉ]CNIC[AO]\s+SUPERIOR\s+EN\s+/i, "")
  ...
  function crearCodigoCarrera(nombre) {
    ...
    return base ? `CERTI-${base}` : "CERTI-CARRERA";
  }
  ...
  function formatearFechaLarga(fecha) {
    ...
    return limpiarTexto(fecha);
  }

  function formatearPromedio(promedio) {
    if (U && typeof U.formatearPromedio === "function") {
      return U.formatearPromedio(promedio);
    }

    const numero = convertirPromedio(promedio);

    return numero === null ? "" : numero.toFixed(3);
  }
