(function() {
    ...
    function obtenerCodigoMemo(data) {
        ...
        if (data.memoCodigoCompleto) {
            return String(data.memoCodigoCompleto).trim();
        }

        if (data.memo && data.memo.codigoCompleto) {
            return String(data.memo.codigoCompleto).trim();
        }

        var correlativo = "01";
        var codigoMes = "";

        if (data.memoCorrelativoMensual) {
            correlativo = pad2(data.memoCorrelativoMensual);
        } else if (data.memo && data.memo.correlativo) {
            correlativo = pad2(data.memo.correlativo);
        } else if (data.memo && data.memo.numero) {
            correlativo = pad2(data.memo.numero);
        }

        if (data.memoCodigoMes) {
            codigoMes = String(data.memoCodigoMes).trim();
        } else if (data.memo && data.memo.codigoMes) {
            codigoMes = String(data.memo.codigoMes).trim();
        }

        if (!codigoMes) {
            var d = new Date();
            codigoMes = d.getFullYear() + "-" + pad2(d.getMonth() + 1);
        }

        return correlativo + " MEM-ITSQMET-UTET-" + codigoMes;
    }
    ...
    function obtenerFechaDocumento(data) {
        ...
        if (data.fechaDocumento) {
            return String(data.fechaDocumento).trim();
        }

        if (data.memo && data.memo.fechaHumana) {
            return String(data.memo.fechaHumana).trim();
        }

        return fechaActualHumana();
    }
    ...
    window.MEMCOM.pdf = {
        generatePdfBytes: async function(data) {
            ...
            // --- VARIABLES DINÁMICAS ---
            const codigoMemo = obtenerCodigoMemo(data);
            const fechaDocumento = obtenerFechaDocumento(data);
            ...
            function drawHeaderLine(label, content) {
                ...
                const contentLines = doc.splitTextToSize(String(content || ""), contentWidth - 25);
                doc.text(contentLines, dataX, cursorY);

                return contentLines.length * 6 + 4;
            }

            cursorY += drawHeaderLine(
                "PARA:",
                `Estudiantes del proceso de titulación - Período ${periodoHumano}`
            );

            cursorY += drawHeaderLine(
                "DE:",
                "Magíster Jefferson Villarreal - Coordinador de Titulación y Eficiencia Terminal"
            );

            cursorY += drawHeaderLine(
                "ASUNTO:",
                `CRONOGRAMA DEL ${tipoProcesoTexto.toUpperCase()} PERÍODO ${periodoHumano.toUpperCase()}`
            );

            cursorY += drawHeaderLine("FECHA:", fechaDocumento);
            ...
            doc.text(`CRONOGRAMA DEL ${tipoProcesoTexto.toUpperCase()}`, width / 2, cursorY, {
                align: "center"
            });
            ...
            if (tableData && tableData.length > 0) {
                ...
                rows.forEach((row, rowIndex) => {
                    ...
                    doc.rect(margin, cursorY, tableWidth, rowHeight, "S");
