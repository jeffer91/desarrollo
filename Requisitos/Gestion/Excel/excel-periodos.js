...
(function(window){
  ...
  function n(value){
    var x = parseInt(String(value == null ? "" : value).trim(), 10);
  ...
  function readLegacy(){
    try{
      var rows = JSON.parse(localStorage.getItem(LEGACY_KEY) || "[]");
  ...
  function emptySnapshot(){
    return {
      meta:{app:"Requisitos", module:"ExcelLocal", version:"1.0.0", createdAt:now(), updatedAt:now()},
      periods:[],
      students:[],
      history:[],
      diagnostics:[]
  ...
  function readSnapshot(){
    if(window.ExcelLocalStorage && typeof window.ExcelLocalStorage.readSnapshot === "function"){
      return window.ExcelLocalStorage.readSnapshot();
    }
    try{
  ...
  function writeSnapshot(snapshot){
    var clean = snapshot || emptySnapshot();
    clean.meta = clean.meta && typeof clean.meta === "object" ? clean.meta : {};
    clean.periods = Array.isArray(clean.periods) ? clean.periods : [];
    ...
    if(window.ExcelLocalStorage && typeof window.ExcelLocalStorage.writeSnapshot === "function"){
      return window.ExcelLocalStorage.writeSnapshot(clean);
    }
    localStorage.setItem(SNAPSHOT_KEY, JSON.stringify(clean));
    return clean;
  }

  function idFromParts(ia, im, fa, fm){
    return ia + "-" + pad2(im) + "__" + fa + "-" + pad2(fm);
  }

  function labelFromParts(ia, im, fa, fm){
    return meses(im) + " " + ia + " a " + meses(fm) + " " + fa;
  }
  ...
  function isValidPeriod(period){
    ...
    if(!value || isCedulaLike(value)){
      return false;
    }
    return clean === "sin periodo" || clean === "sin_periodo" || (/\b(enero|febrero|marzo|abril|mayo|junio|julio|agosto|septiembre|setiembre|octubre|noviembre|diciembre)\b/.test(clean) && /20\d{2}/.test(clean));
  }

  function normalizePeriod(period){
    var source = Object.assign({}, period || {});
    var label = text(source.label || source.periodoLabel || source.periodo || source.id);
    var id = text(source.id || source.periodoId || source.value || idFromParts(source.inicioAnio, source.inicioMes, source.finAnio, source.finMes) || label);
    return Object.assign({}, source, {
      id:id,
      periodoId:id,
      label:label || id,
      periodoLabel:label || id,
      updatedAt:text(source.updatedAt || source.actualizadoEn) || now()
    });
  }
  ...
  function normalizeList(rows){
    ...
    (rows || []).forEach(function(row){
      ...
      if(!isValidPeriod(period) || !key || map[key]){
        return;
      }
      map[key] = true;
      list.push(period);
  ...
  function validate(ia, im, fa, fm){
    ...
    fm = n(fm);
    if(!ia || !fa){
      throw new Error("El año inicial y final son obligatorios.");
    }
    if(im < 1 || im > 12 || fm < 1 || fm > 12){
      throw new Error("Los meses deben estar entre 1 y 12.");
    }
    if((ia * 100 + im) > (fa * 100 + fm)){
      throw new Error("El período inicial no puede ser posterior al final.");
    }
    return {
      inicioAnio:ia,
      inicioMes:im,
      finAnio:fa,
      finMes:fm,
      id:idFromParts(ia, im, fa, fm),
      periodoId:idFromParts(ia, im, fa, fm),
      label:labelFromParts(ia, im, fa, fm),
      periodoLabel:labelFromParts(ia, im, fa, fm)
    };
  }
  ...
  async function crearDesdePartes(ia, im, fa, fm){
    ...
    var key = periodKey(period);
    var index = snapshot.periods.findIndex(function(item){
      return periodKey(item) === key || text(item.id || item.periodoId) === period.id;
    });

    if(index >= 0){
      snapshot.periods[index] = Object.assign({}, snapshot.periods[index], period, {updatedAt:now()});
    }else{
      snapshot.periods.push(Object.assign({}, period, {createdAt:now(), updatedAt:now(), source:"carga"}));
      snapshot.history.push({id:"periodo_" + Date.now(), action:"crearPeriodo", periodoId:period.id, periodoLabel:period.label, fileName:"Carga", totalRows:0, createdAt:now()});
    }

    snapshot.meta = Object.assign({}, snapshot.meta || {}, {
      lastPeriodId:period.id,
      totalPeriods:normalizeList(snapshot.periods).length,
    ...
    try{
      if(window.RequisitosBL && typeof window.RequisitosBL.mirrorSnapshotToCollections === "function"){
        ...
        window.RequisitosBL.notificar("snapshot-changed", {source:"excel-periodos", periodoId:period.id, totalPeriods:snapshot.meta.totalPeriods});
      }
    }catch(error){}
    return period;
  }
  ...
  async function listarTodos(){
    var snapshot = readSnapshot();
    var fromBaseLocal = normalizeList(snapshot.periods || []);
    if(fromBaseLocal.length){
      return fromBaseLocal;
    }
    return normalizeList(readLegacy());
  }
  ...
  window.ExcelPeriodos = {
    crearDesdePartes:crearDesdePartes,
    listarTodos:listarTodos,
    asegurarDemo:asegurarDemo,
    validate:validate
  };
})(window);
