...
(function(window){
  ...
  function readFromSession(){var api = sessionApi();if(!api || typeof api.getSnapshot !== "function"){return null;}try{var snap = api.getSnapshot({clone:false});if(looksLikeSnapshot(snap)){memory.loaded = true;memory.snapshot = normalizeSnapshot(snap);memory.source = "maq-session";return clone(memory.snapshot);}}catch(error){console.warn("[ExcelLocalStorage] sesión rápida no disponible", error);}return null;}

  function readSnapshot(options){
    options = options || {};
    if(options.session !== false){var fromSession = readFromSession();if(fromSession){return fromSession;}}
    try{var raw = localStorage.getItem(key()) || "";if(memory.loaded && memory.raw === raw && memory.snapshot){return clone(memory.snapshot);}var snap = raw ? normalizeSnapshot(JSON.parse(raw)) : emptySnapshot();memory.loaded = true;memory.raw = raw;memory.snapshot = snap;memory.source = "localStorage";return clone(snap);}catch(error){console.warn("[ExcelLocalStorage] lectura fallida", error);var fallback = emptySnapshot();memory.loaded = true;memory.raw = "";memory.snapshot = fallback;memory.source = "fallback";return fallback;}
  }
