...
(function(window,document){
  ...
  function normalizePeriod(period){
    ...
    return {id:id,periodoId:id,label:label,periodoLabel:label,inicioMes:p.inicioMes||null,inicioAnio:p.inicioAnio||null,finMes:p.finMes||null,finAnio:p.finAnio||null,updatedAt:now()};
  }
  ...
  function upsertPeriod(snap,period){
    ...
    var i=snap.periods.findIndex(function(x){return x.id===p.id;});
  ...
  function saveAnalysis(payload){
    ...
    snap.meta.lastPeriodId=period.id;
    snap.meta.lastFileName=text(payload.fileName);
    ...
    try{
      if(window.RequisitosBL&&typeof window.RequisitosBL.mirrorSnapshotToCollections==="function"){
        ...
        window.RequisitosBL.notificar("snapshot-changed",{source:"excel-local.repo",periodoId:period.id,totalStudents:snap.students.length,stats:reconciled.stats||null});
      }
    }catch(error){}
    return saved;
  }
  ...
  function patchStudentById(id,patch){
    ...
    var wanted=text(id);
    var found=false;
    snap.students=snap.students.map(function(s){
      if(text(s._docId||s.docId||s.cedula||s.numeroIdentificacion)===wanted||getCedula(s)===wanted){
        found=true;
        return Object.assign({},s,patch||{},{updatedAt:now(),ultimaSincronizacion:now()});
      }
      return s;
    });
    if(!found)throw new Error("No se encontró estudiante: "+wanted);
    var saved=write(snap);
    try{if(window.RequisitosBL){window.RequisitosBL.mirrorSnapshotToCollections();window.RequisitosBL.notificar("snapshot-changed",{source:"patchStudentById",id:wanted});}}catch(error){}
    return saved;
  }
  ...
  function clearPeriod(periodId){
    ...
    snap.students=snap.students.filter(function(s){return s.periodoId!==id;});
    snap.history.push({id:"clear_"+Date.now(),periodoId:id,action:"clearPeriod",createdAt:now()});
    var saved=write(snap);
    try{if(window.RequisitosBL){window.RequisitosBL.mirrorSnapshotToCollections();window.RequisitosBL.notificar("snapshot-changed",{source:"clearPeriod",periodoId:id});}}catch(error){}
    return saved;
  }
