...
- Aplicar filtros de período, matrícula, carrera, estado y búsqueda.
...
(function(window){
  ...
  function decorate(row){var r=Object.assign({},row||{});r._estadoGeneral=estadoEstudiante(r);r._estadoMatricula=estadoMatricula(r.estadoMatricula);r._cedula=text(r.cedula||r.numeroIdentificacion||r.numeroidentificacion);r._nombres=text(r.nombres||r.Nombres||r.nombre||r.estudiante);r._carrera=text(r.nombrecarrera||r.nombreCarrera||r.NombreCarrera||r.carrera);r._celular=text(r.celular||r.Celular||r.telefono||r.whatsapp);r._correo=text(r.correopersonal||r.CorreoPersonal||r.correoPersonal||r.correoinstitucional||r.CorreoInstitucional||r.correoInstitucional);return r;}
  function filter(opts){opts=opts||{};var q=norm(opts.search);var periodId=text(opts.periodId);var career=text(opts.career);var status=text(opts.status);var matricula=opts.matricula==null?"ACTIVO":text(opts.matricula);var list=students(matricula).map(decorate).filter(function(s){if(matricula&&s._estadoMatricula!==matricula)return false;if(periodId&&s.periodoId!==periodId)return false;if(career&&s._carrera!==career)return false;if(status&&s._estadoGeneral.id!==status)return false;if(q){var hay=norm([s._cedula,s._nombres,s._carrera,s._correo,s._celular,s.periodoLabel,s._estadoMatricula].join(" "));if(hay.indexOf(q)<0)return false;}return true;});return list;}
  ...
  window.TablaCore={REQS:REQS,estadoEstudiante:estadoEstudiante,estadoMatricula:estadoMatricula,periods:periods,students:students,careers:careers,filter:filter,summary:summary,whatsappUrl:whatsappUrl,decorate:decorate};
})(window);
