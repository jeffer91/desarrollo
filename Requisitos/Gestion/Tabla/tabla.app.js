...
- Mantener filtros rápidos por período, matrícula, carrera, estado y búsqueda.
...
(function(window,document){
  "use strict";
  var state={periodId:"",matricula:"ACTIVO",career:"",status:"",search:"",rows:[]};
  ...
  function fillSelects(){var p=el("tabla-periodo");var c=el("tabla-carrera");var m=el("tabla-matricula");var periods=window.TablaCore.periods();var currentCareer=state.career;if(p){p.innerHTML=option("","Todos",!state.periodId)+periods.map(function(x){return option(x.id,x.label||x.id,state.periodId===x.id);}).join("");}if(m){m.value=state.matricula;}if(c){var baseRows=window.TablaCore.filter({periodId:state.periodId,matricula:state.matricula,search:"",status:""});var careers=window.TablaCore.careers(baseRows);c.innerHTML=option("","Todas",!currentCareer)+careers.map(function(x){return option(x,x,currentCareer===x);}).join("");c.value=currentCareer;}}
  ...
  function renderTable(rows){var wrap=el("tabla-table-wrap");if(!wrap)return;if(!rows.length){wrap.innerHTML='<div class="empty">Sin datos. Primero carga un Excel en Carga o cambia el filtro de matrícula.</div>';return;}var shown=rows.slice(0,500);var html='<table><thead><tr><th>Cédula</th><th>Nombre</th><th>Carrera</th><th>Período</th><th>Matrícula</th><th>Estado</th><th>Correo</th><th>Celular</th><th>Acciones</th></tr></thead><tbody>';html+=shown.map(function(r){return '<tr><td class="nowrap">'+esc(r._cedula)+'</td><td>'+esc(r._nombres)+'</td><td>'+esc(r._carrera)+'</td><td>'+esc(r.periodoLabel||r.periodoId)+'</td><td>'+matriculaPill(r)+'</td><td>'+pill(r)+'</td><td>'+esc(r._correo)+'</td><td class="nowrap">'+esc(r._celular)+'</td><td>'+actions(r)+'</td></tr>';}).join('');html+='</tbody></table>';wrap.innerHTML=html;wrap.querySelectorAll('[data-copy]').forEach(function(b){b.addEventListener('click',function(){var value=b.getAttribute('data-copy')||'';if(navigator.clipboard&&navigator.clipboard.writeText)navigator.clipboard.writeText(value);status('Cédula copiada: '+value,'ok');});});}
  function render(){try{fillSelects();state.rows=window.TablaCore.filter(state);var sum=window.TablaCore.summary(state.rows);el("tabla-kpi-total").textContent=sum.total;el("tabla-kpi-ok").textContent=sum.cumple||0;el("tabla-kpi-pend").textContent=sum.pendiente||0;el("tabla-kpi-no").textContent=sum.no_cumple||0;el("tabla-kpi-carreras").textContent=sum.carreras||0;el("tabla-count-text").textContent=sum.total+" registro(s)";renderTable(state.rows);status("Tabla cargada correctamente. Matrícula: "+(state.matricula||"Todos")+".","ok");}catch(e){console.error("[Tabla]",e);status(e.message||String(e),"warn");}}
  function bind(){el("tabla-periodo").addEventListener("change",function(e){state.periodId=e.target.value;state.career="";render();});el("tabla-matricula").addEventListener("change",function(e){state.matricula=e.target.value;state.career="";render();});el("tabla-carrera").addEventListener("change",function(e){state.career=e.target.value;render();});el("tabla-estado").addEventListener("change",function(e){state.status=e.target.value;render();});el("tabla-search").addEventListener("input",function(e){state.search=e.target.value;render();});el("tabla-refresh").addEventListener("click",render);el("tabla-export-csv").addEventListener("click",function(){window.TablaExport.exportCsv(state.rows);});el("tabla-export-json").addEventListener("click",function(){window.TablaExport.exportJson(state.rows);});}
