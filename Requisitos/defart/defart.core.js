...
- Ejecutar diagnóstico final de conexión Defensas ↔ BL ↔ Firebase.
Con qué se conecta:
- excel-local.repo.js
...
(function(window){
  ...
  function writeSnapshot(snapshot){
    var saved;
    if(storage() && typeof storage().writeSnapshot === "function"){saved = storage().writeSnapshot(snapshot);}
    else{saved = snapshot;}
    try{
  ...
  function backupSnapshot(snapshot, reason){
    try{window.localStorage.setItem("REQ_DEFART_BACKUP_V1:" + Date.now(), JSON.stringify({reason:reason || "before_save", createdAt:now(), snapshot:clone(snapshot)}));}
    catch(error){console.warn("[DefartCore] No se pudo crear backup", error);}
  }
  ...
  function noteToText(value){var num = numberValue(value);return num === null ? "" : String(round2(num));}
  ...
  function requirementSummary(row){
    ...
    REQUIREMENTS.forEach(function(req){
      ...
      if(!requirementOk(value)){missing.push(req.label);}
    });
  ...
  function unique(list, getter){
    var map = {};
    (list || []).forEach(function(item){var value = text(getter(item));if(value) map[value] = true;});
    return Object.keys(map).sort(function(a, b){return a.localeCompare(b, "es");});
  }

  function periodOptions(rows){
    var map = {};
    rawPeriods().forEach(function(period){var id = text(period.id || period.periodoId || period.value);if(id) map[id] = {id:id, label:text(period.label || period.periodoLabel || id)};});
    rows.forEach(function(row){if(row._periodoId && !map[row._periodoId]){map[row._periodoId] = {id:row._periodoId, label:row._periodoLabel || row._periodoId};}});
  ...
  function filterRows(options){
    ...
    if(options.sortKey){rows.sort(function(a, b){var result = compareValues(a, b, options.sortKey);return options.sortDir === "desc" ? -result : result;});}
    else{rows.sort(function(a, b){return a._nombre.localeCompare(b._nombre, "es");});}
    return rows;
  }
  ...
  function kpis(rows){
    ...
    rows.forEach(function(row){if(Object.prototype.hasOwnProperty.call(result, row._estadoDefensa)) result[row._estadoDefensa] += 1;});
    return result;
  }
  ...
  function diagnosticsFromRows(allActive, rows, options){
    ...
    return {
      ok:true,
      generatedAt:now(),
      version:"defensas-bloque-5-final",
      source:"BaseLocal",
  ...
  function summary(options){
    ...
    return {rows:rows, kpis:kpis(rows), periodList:periodOptions(allActive), careerList:unique(allActive, function(row){return row._carrera;}), sedeList:unique(allActive, function(row){return row._sede;}), states:STATES.slice(), diagnostics:diagnosticsFromRows(allActive, rows, options)};
  }

  function findStudentIndex(students, id){
    id = text(id);
    for(var i = 0; i < students.length; i += 1){if(studentId(students[i], i) === id) return i;}
    return -1;
  }
  ...
  function validateChange(current, change, patch, nart, ndef){
    ...
    if(Object.prototype.hasOwnProperty.call(change, "nart") && !current._canArt) errors.push("N-ART bloqueada por requisitos: " + current._nombre);
  ...
  function saveNotes(changes){
    ...
    if(saved > 0){
      ...
      snapshot.meta = Object.assign({}, snapshot.meta || {}, {updatedAt:now(), lastDefensasUpdateAt:now(), lastDefensasSaved:saved, lastDefensasPeriodos:periodIds});
      snapshot.history.unshift({id:"defensas_notas_" + Date.now(), action:"guardarNotasDefensas", periodoId:periodIds.length === 1 ? periodIds[0] : "VARIOS", periodoLabel:periodIds.length === 1 ? periodIds[0] : "Varios", fileName:"Defensas", totalRows:saved, errores:errors.length, createdAt:now()});
  ...
  function selfTest(){
    var result = {ok:true, checkedAt:now(), version:"defensas-bloque-5-final", checks:[]};
  ...
  window.DefartCore = {summary:summary, saveNotes:saveNotes, decorate:decorate, preview:preview, noteToText:noteToText, isValidNote:isValidNote, calculateFinal:calculateFinal, selfTest:selfTest, requirements:REQUIREMENTS.slice(), states:STATES.slice()};
})(window);
