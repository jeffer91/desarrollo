...
(function(window){
  ...
  function filtered(opts){opts=opts||{};var periodId=text(opts.periodId);var division=text(opts.division);var career=text(opts.career);var priorityLevel=text(opts.priority);return rawStudents().map(decorate).filter(function(s){if(periodId&&!samePeriod(s._periodoId,periodId))return false;if(division&&!hasDivision(s,division))return false;if(career&&s._carrera!==career)return false;if(priorityLevel&&s._priority.level!==priorityLevel)return false;return true;});}
  ...
  function summary(opts){opts=opts||{};var list=filtered(opts);var k={total:list.length,alta:0,media:0,baja:0,carreras:0};list.forEach(function(s){k[s._priority.level]++;});var careerSummary=byCareer(list);k.carreras=careerSummary.length;var baseForDivision=filtered({periodId:opts.periodId||"",division:"",career:"",priority:""});var baseForCareer=filtered({periodId:opts.periodId||"",division:opts.division||"",career:"",priority:""});return {kpis:k,rows:list.sort(function(a,b){return b._priority.score-a._priority.score||a._nombres.localeCompare(b._nombres,"es");}),carreras:careerSummary,requisitos:byRequirement(list),periodList:periods(),divisionList:divisions(baseForDivision),careerList:careers(baseForCareer),diagnostics:{generatedAt:new Date().toISOString(),filters:opts,total:list.length,divisiones:divisions(baseForDivision)}};}
  ...
  window.CoordiCore={REQS:REQS,periods:periods,careers:careers,divisions:divisions,filtered:filtered,summary:summary,message:message,priority:priority,divisionOf:divisionOf};
})(window);
