...
Función o funciones:
- Leer coordinación primero desde BL2/cache y usar ExcelLocalRepo como respaldo.
...
(function(window){
  ...
  function periods(){if(useBL2())return (bl2().coordiSummary({}).periodList||[]);return repo().listPeriods?repo().listPeriods():repo().getSnapshot().periods||[];}
  ...
  function priority(row){if(useBL2())return bl2().priority(row);var no=0,pend=0,ok=0;REQS.forEach(function(req){var e=estadoCelda(row[req.key]);if(e==="cumple")ok++;else if(e==="no_cumple")no++;else pend++;});var score=no*3+pend;var level=score>=8||no>=2?"alta":score>=3?"media":"baja";return {level:level,label:level==="alta"?"Alta":level==="media"?"Media":"Baja",score:score,ok:ok,no:no,pend:pend,total:REQS.length};}
  function decorate(row){var r=Object.assign({},row||{});r._cedula=text(r.cedula||r.numeroIdentificacion||r.numeroidentificacion);r._nombres=text(r.nombres||r.nombre||r.estudiante);r._carrera=text(r.nombrecarrera||r.nombreCarrera||r.carrera)||"SIN CARRERA";r._division=divisionOf(r);r._periodo=text(r.periodoLabel||r.periodoId)||"SIN PERÍODO";r._periodoId=text(r.periodoId);r._correo=text(r.correopersonal||r.correoPersonal||r.correoinstitucional||r.correoInstitucional);r._celular=text(r.celular||r.telefono||r.whatsapp);r._priority=priority(r);return r;}
  function filtered(opts){opts=opts||{};if(useBL2())return bl2().coordiSummary(opts).rows||[];var periodId=text(opts.periodId);var division=text(opts.division);var career=text(opts.career);var priorityLevel=text(opts.priority);return rawStudents().map(decorate).filter(function(s){if(periodId&&!samePeriod(s._periodoId,periodId))return false;if(division&&!hasDivision(s,division))return false;if(career&&s._carrera!==career)return false;if(priorityLevel&&s._priority.level!==priorityLevel)return false;return true;});}
  ...
  function divisions(list){var rows=list||rawStudents().map(decorate);if(window.BLDivisionesService&&typeof window.BLDivisionesService.listDivisionsWithEmpty==="function")return window.BLDivisionesService.listDivisionsWithEmpty(rows,"");var map={};rows.forEach(function(s){map[divisionOf(s)]=true;});return Object.keys(map).sort(function(a,b){return a.localeCompare(b,"es");});}
