...
(function(window){
  ...
  function build(opts){opts=opts||{};if(opts.matricula==null)opts.matricula="ACTIVO";var list=filtered(opts);var kpis={total:list.length,cumple:0,pendiente:0,no_cumple:0,avance:0};list.forEach(function(s){kpis[s._estado.id]++;});kpis.avance=pct(kpis.cumple,kpis.total);var baseForDivision=filtered({periodId:opts.periodId||"",division:"",matricula:opts.matricula||"",career:""});var baseForCareer=filtered({periodId:opts.periodId||"",division:opts.division||"",matricula:opts.matricula||"",career:""});var data={tipo:text(opts.tipo)||"general",generatedAt:new Date().toISOString(),kpis:kpis,carreras:byCareer(list),requisitos:byRequirement(list),pendientes:pendingStudents(list),periodList:periods(),divisionList:divisions(baseForDivision),careerList:careers(baseForCareer),rows:list,filters:opts};data.text=makeText(data);data.html=makeHtml(data);return data;}
  ...
  window.RepoCore={REQS:REQS,periods:periods,careers:careers,divisions:divisions,build:build,estadoCelda:estadoCelda,estadoGeneral:estadoGeneral,estadoMatricula:estadoMatricula,divisionOf:divisionOf};
})(window);
