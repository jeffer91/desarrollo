...
- Cachear estudiantes normalizados por matrícula para evitar normalizar todo en cada render.
Con qué se conecta:
- excel-local.repo.js
- bl-campos.js
...
(function(window){
  ...
  function periods(){if(cache.periods)return cache.periods.slice();var list=repo().listPeriods?repo().listPeriods():repo().getSnapshot().periods||[];cache.periods=list.slice();return list;}
  ...
  function normalizeStudent(row){var r=Object.assign({},row||{});r._id=text(fieldValue(r,"cedula","")||fieldValue(r,"numeroIdentificacion","")||r._docId||r.docId);r._cedula=text(fieldValue(r,"cedula",fieldValue(r,"numeroIdentificacion","")));r._nombres=text(fieldValue(r,"nombres",r.nombre||r.estudiante||""));r._carrera=text(fieldValue(r,"nombreCarrera",r.carrera||""));r._division=divisionOf(r);r._sede=text(fieldValue(r,"sede",r.Sede||""));r._horario=text(pick(r,["horariocomplexivo","HorarioComplexivo","horarioComplexivo","horario"],""));r._celular=text(fieldValue(r,"celular",r.telefono||r.whatsapp||""));r._correo=text(fieldValue(r,"correoPersonal",fieldValue(r,"correoInstitucional","")));r._periodo=text(fieldValue(r,"periodoLabel",fieldValue(r,"periodoId","")));r._estadoMatricula=estadoMatricula(fieldValue(r,"estadoMatricula",r.estadoMatricula));var tg=telegramInfo(r);r._telegramUser=tg.user;r._telegramChatId=tg.chatId;r._estado=estadoGeneral(r);return r;}
  ...
  function rowsSignature(rows,matricula){var first=rows[0]||{};var last=rows[rows.length-1]||{};return [matricula||"",rows.length,first._docId||first.docId||first.cedula||"",first.updatedAt||"",last._docId||last.docId||last.cedula||"",last.updatedAt||""].join("|");}
  ...
  function invalidate(){cache.periods=null;cache.studentsByMatricula={};cache.signatureByMatricula={};}
  ...
  function buildReq(row,req){var raw=text(reqValue(row,req));var estado=estadoCelda(raw);return {key:req.key,field:req.field,label:req.label,icon:req.icon||"",value:raw||"NO CUMPLE",estado:estado};}
