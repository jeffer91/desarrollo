...
- Calcular Nfin con fórmula institucional cuando Notafinal venga vacío.
...
(function(window){
  ...
  function fieldValue(row, field, fallback){
    ...
    try{
      if(window.BLCampos && typeof window.BLCampos.getValue === "function"){
        return window.BLCampos.getValue(row, field, fallback);
      }
    }catch(error){}
  ...
  function estadoMatricula(v){
    return norm(v || "ACTIVO") === "retirado" ? "RETIRADO" : "ACTIVO";
  }
  ...
  function periods(){
    ...
    if(bl2Students() && typeof bl2Students().listPeriods === "function"){
      ...
      list = repo().listPeriods ? repo().listPeriods() : repo().getSnapshot().periods || [];
    }
  ...
  function rawStudents(matricula){
    if(hasBL2()){
      return bl2Students().listarPagina({matricula:matricula == null ? "ACTIVO" : matricula, limit:5000}).rows || [];
    }
  ...
  function telegramInfo(row){
    var user = text(pick(row, ["telegramUser","TelegramUser","telegramuser","usuarioTelegram","UsuarioTelegram","telegram","Telegram"], ""));
    var chatId = text(pick(row, ["telegramChatId","TelegramChatId","telegramchatid","chatIdTelegram","ChatIdTelegram","chatId","ChatId"], ""));
    return {user:user, chatId:chatId};
  }

  function telegramUrl(row){
    var info = telegramInfo(row);
    if(info.user){return "https://t.me/" + encodeURIComponent(info.user.replace(/^@+/, ""));}
  ...
  function normalizeStudent(row){
    ...
    r._id = text(r._bl2Id || fieldValue(r, "cedula", "") || fieldValue(r, "numeroIdentificacion", "") || r._docId || r.docId);
    r._cedula = text(r._bl2Id || fieldValue(r, "cedula", fieldValue(r, "numeroIdentificacion", "")));
    r._nombres = text(r._bl2Nombre || fieldValue(r, "nombres", r.nombre || r.estudiante || ""));
    r._carrera = text(r._bl2Carrera || fieldValue(r, "nombreCarrera", r.carrera || ""));
    r._division = divisionOf(r);
    r._sede = text(fieldValue(r, "sede", r.Sede || ""));
    r._horario = text(pick(r, ["horariocomplexivo","HorarioComplexivo","horarioComplexivo","horario"], ""));
    r._celular = text(fieldValue(r, "celular", r.telefono || r.whatsapp || ""));
    r._correo = text(fieldValue(r, "correoPersonal", fieldValue(r, "correoInstitucional", "")));
    r._periodo = text(r._bl2Periodo || fieldValue(r, "periodoLabel", fieldValue(r, "periodoId", "")));
  ...
  function estadoGeneral(row){
    ...
    ALL_REQS.forEach(function(req){
      var e = estadoCelda(reqValue(row, req));
      if(e === "cumple"){ok += 1;}
      else if(e === "no_cumple"){no += 1;}
      else{pend += 1;}
    });
    if(no > 0){return {id:"no_cumple", label:"No cumple", ok:ok, no:no, pend:pend};}
    if(pend > 0){return {id:"pendiente", label:"Con pendientes", ok:ok, no:no, pend:pend};}
    return {id:"cumple", label:"Cumple todo", ok:ok, no:no, pend:pend};
  }

  function rowsSignature(rows, matricula){
    var first = rows[0] || {};
    var last = rows[rows.length - 1] || {};
    return [
      matricula || "",
      rows.length,
      first._docId || first.docId || first.cedula || first._bl2Id || "",
      first.updatedAt || first.notasDefensaActualizadasEn || "",
      last._docId || last.docId || last.cedula || last._bl2Id || "",
      last.updatedAt || last.notasDefensaActualizadasEn || ""
    ].join("|");
  }

  function students(matricula){
    var key = matricula == null ? "ACTIVO" : matricula;
    var rows = rawStudents(key);
    var signature = rowsSignature(rows, key);
    if(cache.studentsByMatricula[key] && cache.signatureByMatricula[key] === signature){
      return cache.studentsByMatricula[key];
    }
    var normalized = rows.map(normalizeStudent);
    cache.studentsByMatricula[key] = normalized;
    cache.signatureByMatricula[key] = signature;
    return normalized;
  }

  function invalidate(){
    cache.periods = null;
    cache.studentsByMatricula = {};
    cache.signatureByMatricula = {};
    try{if(window.BL2 && typeof window.BL2.invalidate === "function"){window.BL2.invalidate();}}catch(error){}
  }

  function divisions(list, opts){
    opts = opts || {};
    if(!list && bl2Students() && typeof bl2Students().listDivisions === "function"){
      return bl2Students().listDivisions({periodId:opts.periodId || "", matricula:opts.matricula == null ? "ACTIVO" : opts.matricula});
    }
    var rows = list || students("ACTIVO");
    ...
    var map = {};
    rows.forEach(function(s){map[divisionOf(s)] = true;});
  ...
  function filter(opts){
    ...
    var limit = Math.max(1, Number(opts.limit || 400) || 400);

    if(hasBL2()){
      return (bl2Students().buscar({periodId:periodId, division:division, matricula:matricula, search:opts.search || "", limit:limit}).rows || []).map(normalizeStudent);
    }

    return students(matricula).filter(function(s){
      if(matricula && s._estadoMatricula !== matricula){return false;}
      if(periodId && !samePeriod(s.periodoId || s.ultimoPeriodoId, periodId)){return false;}
      if(division && !hasDivision(s, division)){return false;}
      if(q){
        var hay = norm([s._cedula,s._nombres,s._carrera,s._division,s._correo,s._celular,s._periodo,s._estadoMatricula,s._telegramUser,s._telegramChatId].join(" "));
  ...
  function getById(id, opts){
    ...
    opts = opts || {};
    if(hasBL2()){
      var direct = bl2Students().obtenerPorCedula(wanted, {periodId:opts.periodId || "", division:opts.division || "", matricula:opts.matricula == null ? "" : opts.matricula});
      if(direct){return normalizeStudent(direct);}
    }
    return filter({periodId:opts.periodId || "", division:opts.division || "", matricula:opts.matricula == null ? "" : opts.matricula, search:"", limit:5000}).find(function(s){return text(s._id) === wanted;}) || null;
  }

  function buildReq(row, req){
    if(bl2Reqs() && typeof bl2Reqs().requirement === "function"){
      return bl2Reqs().requirement(row, req);
    }
    var raw = text(reqValue(row, req));
    var estado = estadoCelda(raw);
    return {key:req.key, field:req.field, label:req.label, icon:req.icon || "", value:raw || "NO CUMPLE", estado:estado};
  }
  ...
  function pendientes(row, includeSpecial){
    var source = includeSpecial ? ALL_REQS : REQS;
    return source.map(function(req){return buildReq(row, req);}).filter(function(req){return req.estado !== "cumple";});
  }
  ...
  function notasDesdeServicio(row){
    ...
    var values = {nart:info.nart, ndef:info.ndef, nfin:info.nfin};
    return NOTE_FIELDS.map(function(note){
      var n = values[note.key];
      return {key:note.key, label:note.label, value:n == null ? "—" : String(round2(n)), number:n, estado:estadoNota(n)};
    });
  }
  ...
  function notas(row){
    ...
    var nfin = calcularNfin(nart, ndef);
  ...
  function studentMessage(row){
    ...
    if(faltantes.length){
      ...
      faltantes.forEach(function(req){lines.push("- " + req.label);});
    }else{
