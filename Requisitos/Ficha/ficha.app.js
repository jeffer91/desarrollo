...
(function(window,document){
  ...
  function renderList(){var box=el("ficha-list");el("ficha-count").textContent=String(state.rows.length);if(!state.rows.length){box.innerHTML='<div class="empty-list">Sin estudiantes. Primero carga un Excel en Carga o cambia los filtros.</div>';return;}box.innerHTML=state.rows.slice(0,400).map(function(s){var active=s._id===state.selectedId?' is-active':'';return '<button type="button" class="ficha-item'+active+'" data-id="'+esc(s._id)+'"><strong>'+esc(s._nombres||'Sin nombre')+'</strong><span>'+esc(s._cedula)+' · '+esc(s._carrera)+' · '+esc(s._division||'Sin división')+' · '+esc(s._estadoMatricula)+'</span></button>';}).join('');box.querySelectorAll('[data-id]').forEach(function(btn){btn.addEventListener('click',function(){select(btn.getAttribute('data-id'));});});}
  ...
  function bind(){el("ficha-periodo").addEventListener("change",function(e){state.periodId=e.target.value;state.division="";state.selectedId="";render();});el("ficha-division").addEventListener("change",function(e){state.division=e.target.value;state.selectedId="";render();});el("ficha-matricula").addEventListener("change",function(e){state.matricula=e.target.value;state.division="";state.selectedId="";render();});el("ficha-search").addEventListener("input",function(e){state.search=e.target.value;render();});el("ficha-btn-refresh").addEventListener("click",render);el("ficha-btn-copy").addEventListener("click",function(){var row=selected();if(!row)return;window.FichaExport.copyText(window.FichaCore.toText(row)).then(function(){status("Ficha copiada.","ok");});});el("ficha-btn-json").addEventListener("click",function(){window.FichaExport.exportJson(selected());});el("ficha-copy-cedula").addEventListener("click",function(){var row=selected();if(row)window.FichaExport.copyText(row._cedula).then(function(){status("Cédula copiada.","ok");});});el("ficha-copy-correo").addEventListener("click",function(){var row=selected();if(row)window.FichaExport.copyText(row._correo).then(function(){status("Correo copiado.","ok");});});}
