...
- Evitar repetir en el cuerpo datos ya mostrados en encabezado.
...
(function(window, document){
  ...
  function fillPeriodAndMatricula(){
    ...
    if(sel){
      ...
      sel.innerHTML = option("", "Todos", !state.periodId) + list.map(function(p){return option(p.id, p.label || p.periodoLabel || p.id, state.periodId === p.id);}).join("");
    }
  ...
  function fillDivisionFilter(){
    ...
    if(state.division && !divisions.some(function(x){return x === state.division;})){
      state.division = "";
      div.value = "";
    }else{
  ...
  function renderList(){
    ...
    box.innerHTML = state.rows.slice(0, 400).map(function(s){
      var active = s._id === state.selectedId ? ' is-active' : '';
      return '<button type="button" class="ficha-item' + active + '" data-id="' + esc(s._id) + '"><strong>' + esc(s._nombres || 'Sin nombre') + '</strong><span>' + esc(s._cedula) + ' · ' + esc(s._carrera) + ' · ' + esc(s._division || 'Sin división') + ' · ' + esc(s._estadoMatricula) + '</span></button>';
    }).join('');
  }
  ...
  function renderDetail(row){
    ...
    setText("ficha-correo", row._correo || "—");
    setText("ficha-celular", row._celular || "—");

    var w = window.FichaCore.whatsappUrl(row);
    var wa = el("ficha-whatsapp");
    if(wa){wa.href = w || "#";wa.classList.toggle("is-disabled", !w);wa.title = w ? "Enviar mensaje por WhatsApp" : "Celular no registrado";}

    var tg = el("ficha-telegram");
    var tgUrl = window.FichaCore.telegramUrl ? window.FichaCore.telegramUrl(row) : "";
    if(tg){tg.href = tgUrl || "#";tg.classList.toggle("is-disabled", !tgUrl);tg.title = tgUrl ? "Copiar mensaje y abrir Telegram" : "Telegram no registrado";}
