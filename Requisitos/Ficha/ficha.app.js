...
- Filtrar por matrícula: ACTIVO por defecto, RETIRADO opcional.
Con qué se conecta:
- ficha.core.js
- ficha.export.js
========================================================= */
(function(window,document){
  "use strict";
  var state={periodId:"",matricula:"ACTIVO",search:"",rows:[],selectedId:""};
  ...
  function fillPeriods(){var sel=el("ficha-periodo");if(!sel)return;var list=window.FichaCore.periods();sel.innerHTML=option("","Todos",!state.periodId)+list.map(function(p){return option(p.id,p.label||p.id,state.periodId===p.id);}).join("");var mat=el("ficha-matricula");if(mat)mat.value=state.matricula;}
  function estadoClass(e){return e&&e.id==="cumple"?"ficha-pill-ok":e&&e.id==="no_cumple"?"ficha-pill-bad":"ficha-pill-warn";}
  function renderList(){var box=el("ficha-list");el("ficha-count").textContent=String(state.rows.length);if(!state.rows.length){box.innerHTML='<div class="empty-list">Sin estudiantes. Primero carga un Excel en Carga o cambia el filtro de matrícula.</div>';return;}box.innerHTML=state.rows.slice(0,400).map(function(s){var active=s._id===state.selectedId?' is-active':'';return '<button type="button" class="ficha-item'+active+'" data-id="'+esc(s._id)+'"><strong>'+esc(s._nombres||'Sin nombre')+'</strong><span>'+esc(s._cedula)+' · '+esc(s._carrera)+' · '+esc(s._estadoMatricula)+'</span></button>';}).join('');box.querySelectorAll('[data-id]').forEach(function(btn){btn.addEventListener('click',function(){select(btn.getAttribute('data-id'));});});}
