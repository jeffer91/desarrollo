...
- Subir el cambio a Firebase con el push controlado existente.
Con qué se conecta:
...
(function(window, document){
  ...
  async function saveDivision(){
    ...
    try{
      ...
      if(window.BaseLocalFirebase && typeof window.BaseLocalFirebase.push === "function"){
        await window.BaseLocalFirebase.push();
      }
      closeModal();
      try{window.dispatchEvent(new CustomEvent("requisitos:bl:division-created", {detail:result}));}catch(error){}
      status("División creada: " + name + ". Estudiantes actualizados: " + (result.updated || 0) + ".", "bl-status-ok");
    }catch(error){
  ...
  window.BaseLocalDivisionesUI = {open:openModal, close:closeModal, save:saveDivision, renderCareers:renderCareers};
