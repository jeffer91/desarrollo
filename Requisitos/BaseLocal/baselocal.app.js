...
(function(window,document){
  ...
  function refreshSelectors(view){var sel=el("bl-filter-period");if(!sel)return;var current=sel.value;sel.innerHTML='<option value="">Todos los períodos</option>'+view.periods.map(function(p){return '<option value="'+esc(p.id)+'">'+esc(p.label||p.id)+'</option>';}).join('');sel.value=current;}
  function render(){try{var view=window.BaseLocalAPI.buildView(state.periodId,state.search);refreshSelectors(view);el("bl-kpi-periodos").textContent=view.periods.length;el("bl-kpi-estudiantes").textContent=view.students.length;el("bl-kpi-historial").textContent=view.history.length;el("bl-kpi-carreras").textContent=view.careersCount;el("bl-kpi-estado").textContent="Local";renderPeriods(view);renderStudents(view);renderHistory(view);renderDiagnostics(view);var man=el("bl-manual-text");if(man&&window.BaseLocalManual)man.value=window.BaseLocalManual.getManual();status("BaseLocal cargada correctamente.","bl-status-ok");}catch(e){console.error("[BaseLocal]",e);status(e.message||String(e),"bl-status-warn");}}
  ...
  function copyRefs(){var txt=window.BaseLocalManual?window.BaseLocalManual.getManual():"";navigator.clipboard&&navigator.clipboard.writeText(txt).then(function(){status("Referencias copiadas.","bl-status-ok");}).catch(function(){status("No se pudo copiar automáticamente.","bl-status-warn");});}
