...
- Usar ExcelLocalStorage/BL2DataEngine como fuente principal.
- Mantener colecciones espejo solo como compatibilidad liviana.
- Conservar requisitos en espejos livianos para pantallas antiguas.
- Evitar duplicación pesada de estudiantes en localStorage.
- Invalidar BL2 y cachés cuando cambia la Base Local.
========================================================= */
(function(window){
  "use strict";
  ...
  var MIRROR_STORAGE_VERSION = "light-v3-requirements";
  ...
  function invalidateEngines(){try{if(engine() && typeof engine().invalidate === "function"){engine().invalidate();}}catch(error){}try{if(window.BL2CacheResumen && typeof window.BL2CacheResumen.invalidate === "function"){window.BL2CacheResumen.invalidate();}}catch(error){}try{if(window.BL2 && typeof window.BL2.invalidate === "function"){window.BL2.invalidate({emit:false, source:"RequisitosBL"});}}catch(error){}}
  ...
  function readRawSnapshot(){
    ...
    if(session && typeof session.getSnapshot === "function"){try{var sessionSnap = session.getSnapshot({clone:false});if(sessionSnap && typeof sessionSnap === "object"){return canonicalizeSnapshot(clone(sessionSnap));}}catch(error){}}
    var storage = getStorage();
    var snap = storage ? storage.readSnapshot() : safeParse(localStorage.getItem(SNAPSHOT_KEY), {meta:{app:"Requisitos", module:"BaseLocal", source:"fallback", updatedAt:now()}, periods:[], students:[], history:[], diagnostics:[]});
  ...
  function purgeGeneratedCopies(keepCollection){
    var removed = 0;
    try{for(var i = localStorage.length - 1; i >= 0; i -= 1){var k = localStorage.key(i) || "";var isMirror = k.indexOf(DB_PREFIX) === 0;var isBackup = k.indexOf("REQ_EXCEL_LOCAL_V1:beforeFirebaseSync:") === 0;var isBL2Cache = k.indexOf("REQ_BL2_CACHE_RESUMEN::") === 0;if(isMirror){var collection = k.slice(DB_PREFIX.length);if(collection === keepCollection || collection === "metadata"){continue;}}if(isMirror || isBackup || isBL2Cache){try{localStorage.removeItem(k);removed += 1;}catch(error){}}}}catch(error){}
  ...
  function safeSetItem(k, raw, keepCollection){try{localStorage.setItem(k, raw);return true;}catch(error){if(!isQuotaError(error)){throw error;}purgeGeneratedCopies(keepCollection || "");try{localStorage.setItem(k, raw);return true;}catch(secondError){if(!isQuotaError(secondError)){throw secondError;}return false;}}}
  ...
  function compactForStorage(row){
    ...
    Object.keys(source).forEach(function(key){var value = source[key], low = compact(key);if(low.indexOf("base64") >= 0 || low.indexOf("buffer") >= 0 || low.indexOf("blob") >= 0 || low === "raw" || low === "archivooriginal" || low === "exceloriginal"){return;}if(typeof value === "string" && value.length > 5000){out[key] = value.slice(0,5000) + "…";return;}out[key] = value;});
    return out;
  }
  ...
  function writeCollection(collection, rows){var list = Array.isArray(rows) ? rows : [];var finalList = isLightCollection(collection) ? list.map(function(row){return lightRecord(row, collection);}) : list.map(compactForStorage);var raw = JSON.stringify(finalList);if(safeSetItem(dbKey(collection), raw, collection)){return;}safeSetItem(dbKey(collection), JSON.stringify(finalList.map(function(row){return lightRecord(row, collection);})), collection);}
  ...
  function guardar(collection, record, source, options){options = options || {};if(!collection || !record || typeof record !== "object"){return null;}var rows = readCollection(collection);var sourceRecord = isLightCollection(collection) ? lightRecord(record, collection) : record;var copy = normalizeRecord(collection, sourceRecord, source);var index = rows.findIndex(function(row){return row && row._blId === copy._blId;});if(index >= 0){copy._blCreatedAt = rows[index]._blCreatedAt || copy._blCreatedAt;rows[index] = copy;}else{rows.push(copy);}writeCollection(collection, rows);if(options.silent !== true){signal("changed", {collection:collection, id:copy._blId, source:source || "module", count:rows.length});}return copy;}
  function guardarMuchos(collection, records, source, options){options = options || {};if(!Array.isArray(records) || !collection){return [];}var rows = options.replace === true ? [] : readCollection(collection);var indexById = {};rows.forEach(function(row,index){if(row && row._blId){indexById[row._blId] = index;}});var saved = [];records.forEach(function(record){if(!record || typeof record !== "object"){return;}var sourceRecord = isLightCollection(collection) ? lightRecord(record, collection) : record;var copy = normalizeRecord(collection, sourceRecord, source || "module_many");var index = Object.prototype.hasOwnProperty.call(indexById, copy._blId) ? indexById[copy._blId] : -1;if(index >= 0){copy._blCreatedAt = rows[index]._blCreatedAt || copy._blCreatedAt;rows[index] = copy;}else{indexById[copy._blId] = rows.length;rows.push(copy);}saved.push(copy);});writeCollection(collection, rows);if(options.silent !== true){signal("changed", {collection:collection, source:source || "module_many", count:rows.length, changed:saved.length, bulk:true, replace:options.replace === true});}return saved;}
  ...
  function listar(collection, options){var rows = readCollection(collection);options = options || {};if(options.includeDeleted !== true){rows = rows.filter(function(row){return row && row._blDeleted !== true;});}return rows;}
  ...
  function mirrorSnapshotToCollections(options){
    ...
    var periods = Array.isArray(snapshot.periods) ? snapshot.periods : [], students = Array.isArray(snapshot.students) ? snapshot.students : [];
    var signature = snapshotSignature(snapshot), savedSignature = getSavedMirrorSignature(), mustRebuildForStorage = getMirrorStorageVersion() !== MIRROR_STORAGE_VERSION;
    if(!mustRebuildForStorage && options.force !== true && options.rebuild !== true && signature && (signature === mirrorState.lastSignature || signature === savedSignature)){mirrorState.lastSignature = signature;return {periods:periods.length, students:students.length, defensas:students.length, skipped:true, reason:"same_snapshot_session"};}
    ...
    try{
      ...
      guardarMuchos("estudiantes", students.map(function(row){return normalizer() && normalizer().normalize ? normalizer().normalize(row,{clone:false}) : row;}), "snapshot_mirror", {silent:true, replace:replace});
      ...
      guardar("metadata", {id:"snapshot_mirror_status", totalPeriods:periods.length, totalStudents:students.length, totalDefensas:students.length, updatedAt:now(), signature:signature, rebuild:replace, storageVersion:MIRROR_STORAGE_VERSION, lightMirrors:true, requirementsPreserved:true}, "snapshot_mirror", {silent:true});
  ...
  function conteos(){var snapshot = readRawSnapshot();var result = {collections:COLLECTIONS.length, records:0, byCollection:{}};COLLECTIONS.forEach(function(collection){var total=listar(collection).length;if(collection === "periodos" && Array.isArray(snapshot.periods) && snapshot.periods.length > total){total = snapshot.periods.length;}if(["estudiantes","requisitos","fichas","tabla","stats","coordi","reportes","defensas"].indexOf(collection) >= 0 && Array.isArray(snapshot.students) && snapshot.students.length > total){total = snapshot.students.length;}result.byCollection[collection]=total;result.records += total;});return result;}
  ...
  function conectarModulo(moduleName, options){options = options || {};var collection = options.collection || collectionFor(moduleName), globalName = options.globalName || pascal(moduleName) + "BL", globals = options.globals || [];var api = {module:moduleName, collection:collection, guardar:function(record){return guardar(collection,record,moduleName);}, guardarMuchos:function(records){return guardarMuchos(collection,records,moduleName+"_many");}, reemplazar:function(records){return replaceCollection(collection,records,moduleName+"_replace");}, listar:function(){return listar(collection);}, buscarPorId:function(id){return buscarPorId(collection,id);}, marcarEliminado:function(id){return marcarEliminado(collection,id,moduleName+"_delete");}, capturarAutomatico:function(){return capturarGlobales(collection,globals,"auto_capture_" + moduleName);}};window[globalName]=api;setTimeout(function(){api.capturarAutomatico();signal("module-ready", {module:moduleName, collection:collection, globalName:globalName, count:listar(collection).length});},250);window.addEventListener("load", function(){api.capturarAutomatico();});return api;}
  ...
  try{var mirrorResult = mirrorSnapshotToCollections({silent:true});saveStatus({ok:true, mode:"connector_ready", optimized:true, sessionReady:!!getSession(), defensasMirror:true, rebuildReady:true, lightMirrors:true, requirementsPreserved:true, storageVersion:MIRROR_STORAGE_VERSION, mirrorSkipped:mirrorResult.skipped === true, mirrorReason:mirrorResult.reason || "updated"});}
