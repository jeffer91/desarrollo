...
(function(window, document){
  ...
  function lockActive(){
    var lock = readJson(RUN_LOCK_KEY, {running:false, startedAt:""});
    return lock.running === true && minutesSince(lock.startedAt) < MAX_LOCK_MINUTES;
  }

  function setLock(running){
    return saveJson(RUN_LOCK_KEY, {running:!!running, startedAt:running ? now() : "", updatedAt:now()});
  }

  function scriptUrl(relativeOrAbsolute){
    try{return new URL(relativeOrAbsolute, document.currentScript ? document.currentScript.src : window.location.href).href;}catch(error){return relativeOrAbsolute;}
  }
  ...
  function loadScriptOnce(url, marker){
    return new Promise(function(resolve, reject){
      ...
      if(existing){
        ...
        existing.addEventListener("error", function(){reject(new Error("No se pudo cargar " + url));});
        return;
      }
  ...
  function emit(kind, payload){
    ...
    try{window.dispatchEvent(new CustomEvent("maq:baselocal-background-sync:" + kind, {detail:detail}));}catch(error){}
  }
  ...
  async function run(){
    ...
    if(lockActive()){
      saveStatus({ok:true, mode:"locked", skipped:true, message:"Ya existe una sincronización en segundo plano."});
      return;
    }

    if(navigator.onLine === false){
      saveStatus({ok:false, mode:"offline", skipped:true, message:"Sin internet. La Base Local queda activa y Firebase se intentará después."});
      return;
    }
