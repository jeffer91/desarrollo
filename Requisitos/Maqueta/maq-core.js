...
- Preparar Base Local solo cuando una pantalla realmente la necesita.
- Mantener caché de iframes para que filtros, scroll y formularios no se pierdan al cambiar de pantalla.
- Evitar precarga automática pesada para que Requisitos abra inmediatamente.
...
(function(window,document){
  ...
  function shouldPrepareBaseLocal(moduloId){return !!BL_MODULES[String(moduloId||"")];}
  function preloadAllowed(){try{return window.localStorage.getItem(PRELOAD_FLAG_KEY)==="true";}catch(error){return false;}}

  function ensureBaseLocalReady(options){
    options=options||{};
    if(state.baseLocalReady&&!options.force){return true;}
    try{
      if(window.MAQ_BASELOCAL_SESSION&&typeof window.MAQ_BASELOCAL_SESSION.ensureReady==="function"){
        var status=window.MAQ_BASELOCAL_SESSION.ensureReady({force:options.force===true});
        state.baseLocalReady=true;
        if(U.status){U.status("Base Local lista en memoria: "+(status.students||0)+" estudiante(s).");}
        if(U.memory){U.memory("Base Local en memoria · "+(status.periods||0)+" período(s)");}
        return true;
      }
    }catch(error){console.warn("[MAQ_CORE] Base Local no se pudo preparar",error);if(U.status)U.status("Base Local disponible con modo seguro.");}
    state.baseLocalReady=true;
    return true;
  }
  ...
  function makeFrame(modulo, options){
    options=options||{};
    if(shouldPrepareBaseLocal(modulo&&modulo.id)){ensureBaseLocalReady();}
  ...
  function schedulePreload(){
    ...
    state.preloadEnabled=preloadAllowed();
    if(!state.preloadEnabled){state.preloadFinished=true;if(U.memory)U.memory("Precarga automática pausada · modo rápido activo");return;}
  ...
  function navegarPorModuloId(moduloId){
    var modulo=Registry.buscarPorId?Registry.buscarPorId(moduloId):null;
    if(!modulo){console.error("[MAQ_CORE] Módulo no registrado:",moduloId);if(U.status)U.status("Módulo no registrado: "+moduloId);return;}
    if(shouldPrepareBaseLocal(moduloId)){ensureBaseLocalReady();}
    if(state.moduloActivoId===moduloId){label(modulo.nombre);schedulePreload();return;}
    state.moduloAnteriorId=state.moduloActivoId;state.moduloActivoId=moduloId;saveNav(state.moduloActivoId,state.moduloAnteriorId);ensureFrame(moduloId,{preload:false});hideAll();if(pool[moduloId]&&pool[moduloId].iframe)pool[moduloId].iframe.classList.remove("maq-frame-hidden");label(modulo.nombre);if(U.memory)U.memory("En memoria: "+Object.keys(pool).length+" pantalla(s)"+(state.baseLocalReady?" + Base Local":""));emit("modulo:cambiado",{moduloId:moduloId,modulo:modulo,anteriorModuloId:state.moduloAnteriorId});schedulePreload();}
  ...
  function boot(){var btn=document.getElementById("maq-btn-refresh");if(btn)btn.addEventListener("click",refrescarModuloActivo);var prev=document.getElementById("maq-btn-prev");if(prev)prev.addEventListener("click",pantallaAnterior);if(U.status)U.status("Modo rápido activo. Base Local se prepara cuando se necesita.");} 
  if(document.readyState==="loading")document.addEventListener("DOMContentLoaded",boot);else boot();
  window.MAQ_CORE={state:state,bus:{on:on,emit:emit},router:{navegarPorModuloId:navegarPorModuloId,pantallaAnterior:pantallaAnterior},actions:{refrescarModuloActivo:refrescarModuloActivo,ensureBaseLocalReady:ensureBaseLocalReady,schedulePreload:schedulePreload},performance:{preloadFlagKey:PRELOAD_FLAG_KEY,preloadAllowed:preloadAllowed,shouldPrepareBaseLocal:shouldPrepareBaseLocal}};
})(window,document);
