...
- Manejar filtros por período, división, matrícula, carrera, estado, requisito y búsqueda de estudiante.
...
(function(window,document){
  ...
  var state={periodId:"",division:"",matricula:"ACTIVO",career:"",status:"",requirementKey:"",studentSearch:"",data:null};
  ...
  function fillPeriodOptions(data){
    var p=el("stats-periodo");if(!p)return;
    var list=data.periodList||[];
    p.innerHTML=option("","Selecciona un período",!state.periodId)+list.map(function(item){var value=periodValue(item);var label=periodLabel(item)||value;return option(value,label,state.periodId===value);}).join("");
    p.value=state.periodId;
  }

  function fillSimpleSelect(id,emptyLabel,list,current){
    var node=el(id);if(!node)return current;
    list=list||[];
  ...
  function requirementOptionsHtml(data){
    ...
    if(reqs.length)html+='<optgroup label="Requisitos">'+reqs.map(function(item){return option(item.key,item.label,state.requirementKey===item.key);}).join("")+'</optgroup>';
    if(finals.length)html+='<optgroup label="Aprobación final">'+finals.map(function(item){return option(item.key,item.label,state.requirementKey===item.key);}).join("")+'</optgroup>';
    return html;
  }

  function fillRequirementFilter(data){
    var node=el("stats-requisito");if(!node)return;
    node.innerHTML=requirementOptionsHtml(data||{});
    node.value=state.requirementKey;
    if(state.requirementKey&&node.value!==state.requirementKey){state.requirementKey="";node.value="";}
  }
  ...
  function fillFilters(data){
    ...
    var search=el("stats-student-search");if(search&&search.value!==state.studentSearch)search.value=state.studentSearch;
    fillRequirementFilter(data);
  }

  function bar(label,value,total,extra){
    var percent=pct(value,total);
    return '<div class="stats-bar-row '+esc(extra||"")+'"><div class="stats-bar-label" title="'+esc(label)+'">'+esc(label)+'</div><div class="stats-bar-track"><div class="stats-bar-fill" style="width:'+Math.max(0,Math.min(100,percent))+'%"></div></div><div class="stats-bar-value">'+esc(value)+' / '+esc(percent)+'%</div></div>';
  }

  function renderEstados(data){
    var box=el("stats-estados");var total=data.total||0;if(!box)return;
    box.innerHTML=[bar("Aprobado",data.estados.cumple||0,total,"ok"),bar("No cumple",data.estados.no_cumple||0,total,"bad")].join("");
    setText("stats-estados-meta",total+" estudiantes");
  }

  function renderRequisitos(data){
    var box=el("stats-requisitos");if(!box)return;
    var rows=data.requisitos||[];
    if(!rows.length){box.innerHTML='<div class="empty">Sin datos.</div>';return;}
    box.innerHTML=rows.map(function(r){var total=r.aplica||r.total||0;var suffix=r.no_aplica?'<small>No aplica: '+esc(r.no_aplica)+'</small>':'';return '<div>'+bar(r.label,r.cumple,total)+' '+suffix+'</div>';}).join("");
  }

  function table(rows,kind){
    rows=rows||[];
    if(!rows.length)return '<div class="empty">Sin datos.</div>';
    var html='<table class="stats-sortable-table" data-sortable="true"><thead><tr><th data-sort-type="text">Nombre</th><th data-sort-type="number">Total</th><th data-sort-type="number">Cumple</th><th data-sort-type="number">No cumple</th>';
    ...
    html+=rows.map(function(r){var key=esc(r.label||r.key||"Sin dato");var total=num(r.aplica||r.total);var cumple=num(r.cumple);var no=num(r.no_cumple);var noAplica=num(r.no_aplica);var avance=num(r.avance);return '<tr><td data-sort="'+key+'"><strong>'+key+'</strong></td><td data-sort="'+total+'">'+total+'</td><td data-sort="'+cumple+'"><span class="pill pill-ok">'+cumple+'</span></td><td data-sort="'+no+'"><span class="pill pill-bad">'+no+'</span></td>'+((kind==="requirements"||kind==="final")?'<td data-sort="'+noAplica+'"><span class="pill pill-na">'+noAplica+'</span></td>':'')+'<td data-sort="'+avance+'">'+avance+'%</td></tr>';}).join("");
    return html+'</tbody></table>';
  }

  function renderTables(data){setHtml("stats-carreras",table(data.carreras,"general"));setHtml("stats-periodos",table(data.periodos,"general"));setText("stats-carreras-meta",(data.carreras||[]).length+" carreras");setText("stats-periodos-meta",(data.periodos||[]).length+" períodos");}
  function renderFinales(data){setHtml("stats-finales",table(data.requisitosFinales||[],"final"));}

  function renderPeriodApproval(data){
    var chip=el("stats-period-type");var card=el("stats-period-approval-card");var okArticle=el("stats-ok")&&el("stats-ok").closest?el("stats-ok").closest("article"):null;var noArticle=el("stats-no")&&el("stats-no").closest?el("stats-no").closest("article"):null;var hasPeriod=!!state.periodId;
    setVisible(card,hasPeriod);setVisible(okArticle,hasPeriod);setVisible(noArticle,hasPeriod);
    if(!hasPeriod){if(chip)chip.textContent="Selecciona un período";setText("stats-period-approval","—");return;}
  ...
  function renderKpis(data){setText("stats-total",data.total||0);setText("stats-ok",data.estados&&data.estados.cumple?data.estados.cumple:0);setText("stats-pend",0);setText("stats-no",data.estados&&data.estados.no_cumple?data.estados.no_cumple:0);setText("stats-avance",(data.avanceGeneral||0)+"%");renderPeriodApproval(data);}
  function renderCharts(data){if(window.StatsCharts&&typeof window.StatsCharts.renderAll==="function")window.StatsCharts.renderAll(data);}
  function renderNotes(data){if(window.StatsNotes&&typeof window.StatsNotes.render==="function")window.StatsNotes.render(data,"stats-notes");}
  ...
  function render(){
    ...
    }catch(error){console.error("[Stats]",error);status(error.message||String(error),"warn");}
  }
