...
- Manejar filtros por período, división, matrícula, carrera, estado, KPIs, barras, tablas y exportación.
...
(function(window,document){
  "use strict";
  var state={periodId:"",division:"",matricula:"ACTIVO",career:"",status:"",data:null};
  ...
  function fillFilters(data){
    var p=el("stats-periodo"), d=el("stats-division"), c=el("stats-carrera"), m=el("stats-matricula");
    if(p){p.innerHTML=option("","Todos",!state.periodId)+(data.periodList||[]).map(function(x){return option(x.id,x.label||x.periodoLabel||x.id,state.periodId===x.id);}).join("");}
    if(m){m.value=state.matricula;}
    if(d){d.innerHTML=option("","Todas",!state.division)+(data.divisionList||[]).map(function(x){return option(x,x,state.division===x);}).join("");if(state.division&&!(data.divisionList||[]).some(function(x){return x===state.division;})){state.division="";d.value="";}else{d.value=state.division;}}
    if(c){c.innerHTML=option("","Todas",!state.career)+(data.careerList||[]).map(function(x){return option(x,x,state.career===x);}).join("");if(state.career&&!(data.careerList||[]).some(function(x){return x===state.career;})){state.career="";c.value="";}else{c.value=state.career;}}
  }
