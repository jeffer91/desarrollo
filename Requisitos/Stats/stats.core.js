...
Función o funciones:
- Leer estadísticas primero desde BL2/cache y usar ExcelLocalRepo como respaldo.
...
(function(window){
  ...
  function useBL2(){return !!(bl2()&&typeof bl2().resumen==="function");}
  ...
  function estadoMatricula(v){if(useBL2())return bl2().estadoMatricula(v);return norm(v||"ACTIVO")==="retirado"?"RETIRADO":"ACTIVO";}
  function estadoCelda(v){if(useBL2())return bl2().estadoCelda(v);var k=norm(v);if(!k)return "pendiente";if(["si","s","ok","cumple","aprobado","aprobada","1","true","x","validado","completo"].indexOf(k)>=0)return "cumple";if(["no","n","no cumple","reprobado","reprobada","0","false","falta","incompleto"].indexOf(k)>=0)return "no_cumple";return "pendiente";}
  function estadoGeneral(row){if(useBL2())return bl2().estadoGeneral(row);var ok=0,no=0,pend=0;REQS.forEach(function(req){var e=estadoCelda(row[req.key]);if(e==="cumple")ok++;else if(e==="no_cumple")no++;else pend++;});if(no>0)return {id:"no_cumple",label:"No cumple",ok:ok,no:no,pend:pend};if(pend>0)return {id:"pendiente",label:"Con pendientes",ok:ok,no:no,pend:pend};return {id:"cumple",label:"Cumple todo",ok:ok,no:no,pend:pend};}
  ...
  function periods(){if(useBL2()){var data=bl2().resumen({matricula:"ACTIVO"});return data.periodList||[];}return repo().listPeriods?repo().listPeriods():repo().getSnapshot().periods||[];}
  ...
  function divisionOf(row){if(useBL2())return bl2().divisionOf(row);if(window.BLDivisionesService&&typeof window.BLDivisionesService.studentDivision==="function")return window.BLDivisionesService.studentDivision(row);var list=Array.isArray(row&&row.divisiones)?row.divisiones:[];return list[0]||row.division||"Sin división";}
  ...
  function filtered(opts){opts=opts||{};if(useBL2())return bl2().rows(opts);var periodId=text(opts.periodId);var division=text(opts.division);var career=text(opts.career);var status=text(opts.status);var matricula=opts.matricula==null?"ACTIVO":text(opts.matricula);return rawStudents(matricula).map(decorate).filter(function(s){if(matricula&&s._estadoMatricula!==matricula)return false;if(periodId&&!samePeriod(s._periodoId,periodId))return false;if(division&&!hasDivision(s,division))return false;if(career&&s._carrera!==career)return false;if(status&&s._estado.id!==status)return false;return true;});}
  function careers(list){if(useBL2()&&!list){return (bl2().resumen({matricula:"ACTIVO"}).careerList||[]);}var map={};(list||rawStudents("ACTIVO").map(decorate)).forEach(function(s){map[s._carrera]=true;});return Object.keys(map).sort(function(a,b){return a.localeCompare(b,"es");});}
  function divisions(list){if(useBL2()&&!list){return (bl2().resumen({matricula:"ACTIVO"}).divisionList||[]);}var rows=list||rawStudents("ACTIVO").map(decorate);if(window.BLDivisionesService&&typeof window.BLDivisionesService.listDivisionsWithEmpty==="function")return window.BLDivisionesService.listDivisionsWithEmpty(rows,"");var map={};rows.forEach(function(s){map[divisionOf(s)]=true;});return Object.keys(map).sort(function(a,b){return a.localeCompare(b,"es");});}
