...
(function(window,document){
  ...
  function lower(value){return text(value).toLocaleLowerCase("es");}

  function empty(message){
    return '<div class="empty">'+esc(message||"Sin estudiantes para mostrar.")+'</div>';
  }

  function studentName(row){
    return text(row&&row._nombres) || text(row&&row.nombres) || text(row&&row.Nombres) || text(row&&row.nombre) || text(row&&row.Nombre) || text(row&&row.estudiante) || text(row&&row.Estudiante) || "Sin nombre";
  }

  function studentId(row){
    return text(row&&row._cedula) || text(row&&row.cedula) || text(row&&row.Cedula) || text(row&&row.numeroIdentificacion) || text(row&&row.numeroidentificacion) || text(row&&row.identificacion) || "";
  }

  function studentCareer(row){
    return text(row&&row._carrera) || text(row&&row.nombrecarrera) || text(row&&row.nombreCarrera) || text(row&&row.NombreCarrera) || text(row&&row.carrera) || "SIN CARRERA";
  }

  function requirementLabels(list){
    return (list||[]).map(function(item){
      return {
        key:text(item&&item.key),
        label:text(item&&item.label)||text(item&&item.key)
      };
  ...
  function statusHtml(row){
    ...
    if(selected){
      if(selected.status==="no_aplica"){
        return '<span class="student-status-na">'+esc(selected.labelStatus||"No aplica")+'</span>';
      }
      if(selected.cumple){
        return '<span class="student-status-ok">Cumple</span>';
      }
    ...
    return '<div class="student-missing-list">'+missing.map(function(item){
      return '<span class="student-missing" data-key="'+esc(item.key)+'">'+esc(item.label)+'</span>';
    }).join(' ')+'</div>';
  }
  ...
  function statusSortValue(row){
    var selected=row&&row._selectedRequirementStatus;
    if(selected){
      if(selected.status==="no_aplica")return "2-no-aplica";
      return selected.cumple?"1-cumple":"0-no-cumple";
    }
    return row&&row._estado&&row._estado.id==="cumple"?"1-aprobado":"0-no-cumple";
  }
  ...
  function rowHtml(row){
    var name=studentName(row);
    var id=studentId(row);
    var career=studentCareer(row);
    return '<tr>'
  ...
  function tableHtml(rows){
    ...
    return '<table class="stats-sortable-table stats-students-table" data-sortable="true">'
      ...
      + '<th data-sort-type="text">Cédula</th>'
      + '<th data-sort-type="text">Carrera</th>'
  ...
  function render(data,targetId){
    ...
    if(meta)meta.textContent=rows.length+" estudiantes";
    if(window.StatsTables&&typeof window.StatsTables.bindAll==="function")window.StatsTables.bindAll(target);
  }

  window.StatsStudents={
    render:render,
    tableHtml:tableHtml,
    normalizeRows:normalizeRows,
    helpers:{studentName:studentName,studentId:studentId,studentCareer:studentCareer,missingFromRow:missingFromRow,statusHtml:statusHtml,statusSortValue:statusSortValue}
  };
})(window,document);
