...
- Unir anexos sin duplicarlos por id, path, fullPath, readableUrl, publicUrl o nombre.
========================================================= */
...
(function (window) {
  ...
  function dedupeAnexos(anexos) {
    var list = Array.isArray(anexos) ? anexos : [];
    var out = [];
    var seen = {};

    list.forEach(function (item, index) {
      var key = anexoKey(item, index);
  ...
  function mergeAnexos(documentData) {
    ...
    doc.anexos = dedupeAnexos(existing.concat(fromUi));
